...
func (r *TodoItemPostgres) Create(listId int, item todo.TodoItem) (int, error) {
	...
	//EXEC: Without return information. Examples for SQL requests: INSERT, UPDATE
...
func (r *TodoItemPostgres) GetAll(userId, listId int) ([]todo.TodoItem, error) {
	...
	query := fmt.Sprintf(`SELECT ti.id, ti.title, ti.description, ti.update_done FROM %s ti INNER JOIN %s li on li.item_id = ti.id
...
func (r *TodoItemPostgres) Update(userId, itemId int, input todo.UpdateItemInput) error {
	setValues := make([]string, 0) // slice strings
	args := make([]interface{}, 0) // slice interfaces
	argId := 1                     // id argument
